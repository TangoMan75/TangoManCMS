...
class PrivilegesCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName('tangoman:user:privileges')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $superAdmin = $em->getRepository('UserBundle:Role')->findOneBy(['type' => 'ROLE_SUPER_ADMIN']);

        for ($i = 0; $i < count($privileges); $i = $i + 4) {
            if (!$em->getRepository('UserBundle:Privilege')->findBy(['privilege' => $privileges[$i + 3]])) {

                $role = $em->getRepository('UserBundle:Role')->findOneBy(['type' => $privileges[$i + 3]]);
